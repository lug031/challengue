...
/**
 * Inicializa el esquema H2 de Pedidos en el perfil {@code test}.
 * Se usa JDBC para poblar la misma base en memoria que R2DBC.
 */
@Component
@Profile("test")
public class OrdersTestDatabaseSeeder implements ApplicationRunner {
    ...
    public OrdersTestDatabaseSeeder(
            @Value("classpath:orders/schema.sql") Resource schema,
            @Value("classpath:orders/data.sql") Resource data,
    ...
    public void run(ApplicationArguments args) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new IllegalStateException("No se pudo inicializar la base de datos de Orders (test)", ex);
        }
    }
